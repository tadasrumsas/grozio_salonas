...
import ReviewsOneTour from './ReviewsOneTour';
...
export default function TourDetails() {
  ...
  const handleRegister = async () => {
    ...
    const selectedDateObj = tour.dates.find((d) => d.date_time === selectedDate);
    if (!selectedDateObj) {
    ...
    try {
      const response = await axios.post(
        ...
        {
          tour_date_id: selectedDateObj.id
        },
        {
          withCredentials: true,
        }
  ...
  if (error && !tour) return <div>{error}</div>;
  if (!tour) return <div>Procedūra nerasta.</div>;
    ...
    <section>
      ...
      <div className='max-w-7xl mx-auto p-4'>
        ...
        <div className='flex w-full flex-col md:flex-row bg-white border-2 border-gray-200 rounded-xl shadow-md hover:shadow-lg transition-all duration-300'>
          ...
          <div className='w-full md:w-1/2 p-4'>
            <h2 className='font-bold text-2xl sm:text-3xl text-gray-800 truncate'>{tour.title}</h2>
            <p className='text-gray-600 text-base mt-2 leading-relaxed'>{tour.description}</p>
          </div>
        </div>

        {/* Date selection */}
        <div className='mt-4'>
          <label htmlFor='date-select' className='block text-gray-700 text-sm font-semibold mb-1'>
            Pasirinkite datą:
          </label>
          <select
            id='date-select'
